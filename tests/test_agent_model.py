...
class AgentModelTest(unittest.TestCase):
    ...
    def test_missing_config_uses_native_harnesses(self) -> None:
        expected = {
            "claude": "claude",
    ...
    def test_configured_omp_routes_are_subscription_selectors(self) -> None:
        expected = {
    ...
    def test_explicit_harness_wins_without_changing_config(self) -> None:
        self.assertEqual(self.run_model("default", "set", "codex", "omp").returncode, 0)
        result = self.run_model("resolve", "codex", "--harness", "self", "resume")
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(result.stdout.strip(), "codex resume")
        self.assertIn('codex = "omp"', self.config().read_text())
    ...
    def test_short_harness_option_is_recognized_only_for_a_known_harness(self) -> None:
        ...
        self.assertEqual(bare_help.stdout.strip(), "kimi -m kimi-code/k3 -h")

        native_help = self.run_model("resolve", "kimi", "-h", "topic")
        self.assertEqual(native_help.returncode, 0, native_help.stderr)
        self.assertEqual(native_help.stdout.strip(), "kimi -m kimi-code/k3 -h topic")
    ...
    def test_launch_reports_a_missing_harness_executable(self) -> None:
        result = self.run_model("launch", "codex")
        self.assertEqual(result.returncode, 2)
    ...
    def test_default_cli_sets_gets_lists_and_resets(self) -> None:
        set_result = self.run_model("default", "set", "codex", "omp")
        self.assertEqual(set_result.returncode, 0, set_result.stderr)
        self.assertEqual(set_result.stdout.strip(), "codex = omp")
        self.assertIn("version = 1", self.config().read_text())

        get_result = self.run_model("default", "get", "codex")
        self.assertEqual(get_result.stdout.strip(), "codex = omp (config)")
        list_result = self.run_model("default", "list")
        self.assertIn("codex   omp       codex      config", list_result.stdout)

        reset_result = self.run_model("default", "reset", "codex")
        self.assertEqual(reset_result.stdout.strip(), "codex = codex (built-in)")
